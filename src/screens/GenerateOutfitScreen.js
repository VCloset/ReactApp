  ...
  const handleGenerateOutfit = async () => {
    ...
    try {
      ...
      if (description) {
        ...
        response = await axios.post('https://vcloset.xyz/api/generateOutfitsWithDescription', formData, {
            headers: {
      ...
      const topResponse = await axios.get(`https://vcloset.xyz/api/items/${top}`, {
        headers: {
            'Authorization': `Bearer ${accessToken}`,
            },
        });
      ...
      const bottomResponse = await axios.get(`https://vcloset.xyz/api/items/${bottom}`, {
        headers: {
            'Authorization': `Bearer ${accessToken}`,
            },
        });
    ...
    <View style={styles.container}>
      ...
      {generatedOutfit && (
        <View style={styles.generatedOutfit}>
          ...
          {topImage && <Image source={{ uri: `data:image/png;base64,${topImage}` }} style={styles.outfitImage} />}
          <Text style={styles.outfitText}>Bottom: {generatedOutfit.bottom}</Text>
          {bottomImage && <Image source={{ uri: `data:image/png;base64,${bottomImage}` }} style={styles.outfitImage} />}
          <Text style={styles.outfitDescription}>{generatedOutfit.description}</Text>
        </View>
      )}
    </View>
  );
