...
const ShareGenerateOutfitScreen = () => {
  ...
  // to int
  const [share_closet_id, setShare_closet_id] = useState('') // '1
    ...
    try {
      ...
      if (description) {
        ...
        response = await axios.post(
          'https://vcloset.xyz/api/generateOutfitsWithDescription/'+closet,
          formData,
          {
            headers: {
        ...
        response = await axios.get('https://vcloset.xyz/api/generateOutfits/'+closet, {
          headers: {
            Authorization: `Bearer ${accessToken}`,
          },
        })
...
export default ShareGenerateOutfitScreen
